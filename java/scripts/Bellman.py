...
public class BellmanFordNavigator {{
    ...
    for dx, dy in offsets_by_distance:
        ...
        boolean canVisit{id} = rc.canMove({direction(dx, dy)});
        """.rstrip() + "\n"
    ...
    for dx, dy in offsets_by_distance:
        ...
        boolean canVisit{id} = rc.canSenseLocation(location{id}) && (rc.sensePassability(location{id}));
            """.rstrip()

        content += f"""
        int distance{id} = 1_000_000;
        Direction direction{id} = null;
        int weight{id} = canVisit{id} ? (checkOpponents ? rc.senseNearbyRobots(location{id}, 10, opponentTeam).length : 0) : 1_000_000;
        """.rstrip() + "\n"
    ...
    output_file = Path(__file__).parent.parent / "src" / f"BellmanFordNavigator.java"
