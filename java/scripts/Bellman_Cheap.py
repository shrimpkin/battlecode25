...
public class ConservePaintBellmanFordNavigator extends Globals {{
    ...
    for dx, dy in offsets_by_distance:
        ...
        boolean canVisit{id} = rc.canSenseLocation(location{id}) && (rc.sensePassability(location{id})) && (rc.senseMapInfo(location{id}).getPaint().isAlly());
            """.rstrip()

        content += f"""
        int distance{id} = 1_000_000;
        Direction direction{id} = null;
        int weight{id} = canVisit{id} ? 1 : 1_000_000;
        """.rstrip() + "\n"
    ...
    output_file = Path(__file__).parent.parent / "src" / f"BellmanFordNavigatorCheap.java"
