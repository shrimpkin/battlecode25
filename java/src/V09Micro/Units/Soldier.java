...
public class Soldier extends Unit {
    ...
    static boolean debugAttackTarget = true;
    public static void updateAttackTarget() throws GameActionException {

        if(debugAttackTarget) indicator += "AM:";
        if(enemyTower == null) {
            if(debugAttackTarget) indicator += "{null}";
            attackTarget = null;
            return;
        } else if(enemyTower.distanceSquaredTo(rc.getLocation()) > 16) {
            if(debugAttackTarget) indicator += "{walk}";
            attackTarget = enemyTower;
            return;
        } else {
            if(debugAttackTarget) indicator += "{micro}";
    ...
    public static void debug() {
        ...
        if(buildTarget != null) {
            indicator += bMode + " at: " + buildTarget.toString();
        } else if(attackTarget != null) {
            indicator += "Attack at: " + attackTarget.toString();
        } else {
