...
import V08Buggy.Micro.MopperMicro;
import V08Buggy.Nav.Navigator;
import V08Buggy.Tools.CommType;
...
public class Mopper extends Unit {
    ...
    public static void read() throws GameActionException {
        ...
        for (Message msg : msgs) {
            if (Comms.getType(msg.getBytes()) == CommType.TargetEnemy) {
                ...
                if (rc.getLocation().distanceSquaredTo(enemyLoc) <= 5 && rc.canMopSwing(rc.getLocation().directionTo(enemyLoc))) {
    ...
    public static void doAction() throws GameActionException {
        if (!rc.isActionReady() || rc.getPaint() < 30 && refillingTower != null) return;
    ...
    public static void updateSurroundings() throws GameActionException {
        ...
        for (var tower : rc.senseNearbyRobots(GameConstants.PAINT_TRANSFER_RADIUS_SQUARED, myTeam)) {
            if (tower.getType().isTowerType() && (tower.getPaintAmount() >= 50 - rc.getPaint() || rc.getPaint() <= 10)) {
