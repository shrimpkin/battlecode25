package V08Buggy.Units;

import V08Buggy.Comms;
import V08Buggy.Nav.Navigator;
import V08Buggy.Tools.FastIntSet;
import V08Buggy.Tools.FastLocSet;
import V08Buggy.Unit;
...
public class Soldier extends Unit {
    ...
    static FastLocSet ignore = new FastLocSet();
    ...
    public static void run() throws GameActionException {
        ...
        if (mode == Modes.BOOM) {
            boolean painted = paintBuildTarget();
            boolean complete = completeBuiltTarget();
            if (painted && !complete && buildTarget != null) {
    ...
    public static void findValidTowerPosition() throws GameActionException {
        ...
        for (MapLocation ruin : ruinLocations) {
            if (ignore.contains(ruin)) continue;
    ...
    /// Paints incorrectly painted tiles around our build target -- returns whether the entire thing has been built
    public static boolean paintBuildTarget() throws GameActionException {
        //We have not decided on a build target, so we can't paint it
        if(buildTarget == null || bMode == BuildMode.NONE) return false;
        ...
        var infos = rc.senseNearbyMapInfos(buildTarget, 8);
        for(MapInfo info : infos) {
            MapLocation loc = info.getMapLocation();
        ...
        // all locations have been checked against, and are as desired
        return infos.length == 25;
    }

    /// Will complete the SRP and tower pattern at build target if possible -- returns whether its complete
    public static boolean completeBuiltTarget() throws GameActionException {
        //no build target to complete
        if(buildTarget == null || bMode == BuildMode.NONE) return false;
        //haven't decided on tower type yet
        if(bMode == BuildMode.BUILD_TOWER && buildType == null) return false;
        if (!rc.canSenseLocation(buildTarget)) return false;

        boolean completed = false;
        if(bMode == BuildMode.BUILD_TOWER) {
            //a tower has been built or we can complete the tower
            boolean robotIsNull = rc.canSenseRobotAtLocation(buildTarget);
            ...
            if(isCenter ||  (rc.canCompleteResourcePattern(buildTarget) && rc.getMoney() >= 200)) {
                if(!isCenter) {
                    rc.completeResourcePattern(buildTarget);
                    completed = true;
                }
    ...
    public static void updateCommTarget() throws GameActionException {
        ...
        if (commTarget != null && rc.canSenseLocation(commTarget))
            commTarget = null;
    }
