...
import V09Explore.Nav.Navigator;
import V09Explore.Tools.*;
...
public class Unit extends Globals {
    ...
    public static void updateExploreTargets() throws GameActionException {
        for (var target : exploreTargets.getKeys()) {
            if(rc.canSenseLocation(target) && !rc.senseMapInfo(target).isPassable()) {
                exploreTargets.remove(target);
            } else if(rc.getLocation().distanceSquaredTo(target) < 4) {
                exploreTargets.remove(target);
            }
        }
    }
    ...
    public static MapLocation getExploreTarget() throws GameActionException {
        ...
        if (best != null) {
            rc.setIndicatorLine(rc.getLocation(), new MapLocation(0,0), 0,0,0);
            rc.setIndicatorLine(rc.getLocation(), best, 0, 0, 0);
            System.out.println("Using prebuilt");
            return best;
        }
    ...
    public static MapLocation getExploreTargetClose() throws GameActionException{
